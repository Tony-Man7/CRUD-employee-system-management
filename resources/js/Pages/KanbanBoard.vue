...
<template>
  <Layout>
    ...
    <div class="container mx-auto my-8">
      <h2 class="text-center text-3xl font-bold mb-6 text-gray-800">Kanban Board</h2>
      <div class="flex space-x-6" id="kanban-board">
        <!-- To Do Column -->
        <div class="w-1/3">
          <div class="bg-white shadow-lg rounded-lg">
            <div class="bg-blue-600 text-white p-4 rounded-t-lg">
              <div class="flex justify-between items-center">
                <div class="text-lg font-semibold">To Do</div>
                <button @click="showTaskModal('todo')"
                  class="bg-teal-500 text-white px-4 py-2 rounded-full transition hover:bg-teal-600">
                  +
                </button>
              </div>
            </div>
            <div class="p-4 space-y-4">
              <div v-for="item in todoTasks" :key="item.id" class="kanban-item bg-white shadow-md rounded-lg mb-2 p-4">
                <div class="flex justify-between">
                  <div class="text-sm font-medium text-gray-900">{{ item.name }}</div>
                  <div class="space-x-2">
                    <button @click="editTask(item)"
                      class="bg-yellow-500 text-white px-3 py-1 rounded-full text-sm transition hover:bg-yellow-600">
                      <i class="bx bx-comment-edit"></i>
                    </button>
                    <button @click="deleteTask(item.id)"
                      class="bg-red-500 text-white px-3 py-1 rounded-full text-sm transition hover:bg-red-600">
          ...
          <div class="bg-white shadow-lg rounded-lg">
            <div class="bg-yellow-500 text-white p-4 rounded-t-lg">
              <div class="flex justify-between items-center">
                ...
                <button @click="showTaskModal('in_progress')"
                  class="bg-teal-500 text-white px-4 py-2 rounded-full transition hover:bg-teal-600">
                  +
                </button>
              </div>
            ...
            <div class="p-4 space-y-4">
              <div v-for="item in inProgressTasks" :key="item.id"
                class="kanban-item bg-white shadow-md rounded-lg mb-2 p-4">
                <div class="flex justify-between">
                  <div class="text-sm font-medium text-gray-900">{{ item.name }}</div>
                  <div class="space-x-2">
                    <button @click="editTask(item)"
          ...
          <div class="bg-white shadow-lg rounded-lg">
            <div class="bg-green-500 text-white p-4 rounded-t-lg">
              <div class="flex justify-between items-center">
                ...
                <button @click="showTaskModal('done')"
                  class="bg-teal-500 text-white px-4 py-2 rounded-full transition hover:bg-teal-600">
                  +
                </button>
              </div>
            ...
            <div class="p-4 space-y-4">
              <div v-for="item in doneTasks" :key="item.id" class="kanban-item bg-white shadow-md rounded-lg mb-2 p-4">
                <div class="flex justify-between">
                  <div class="text-sm font-medium text-gray-900">{{ item.name }}</div>
                  <div class="space-x-2">
                    <button @click="editTask(item)"
      ...
      <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
          ...
          <input v-model="taskName" type="text" class="w-full px-4 py-2 border rounded mb-4" placeholder="Task Name" />
